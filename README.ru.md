...
- **Стиль Breeze**: устанавливается как `PlasmaBreezeCustom` (чтобы его не перекрывал ни системный `Breeze`, ни
  `PlasmaBreeze` нашего же пакета — это важно для локальных сборок), настраиваемая высота клавиатуры,
...
- **Темы**: десять встроенных тем (system, светлая/тёмная, iOS, Material и Google в светлом и тёмном вариантах, а также Catppuccin Mocha) плюс
...
- **Предиктивный ввод (подсказки слов)**: над клавиатурой предлагаются слова для того, что набирается, — самые частые
  первыми (до трёх одновременно, настраивается 1–5). Работают три вещи. Пока набирается слово, предлагаются слова, которые
  его продолжают (около 1,18 млн русских и 1,03 млн английских словоформ из частотных словарей FrequencyWords, лицензия
  MIT); если набранное не является началом ни одного слова, предлагается исправление опечатки — слова в одной опечатке от
  набранного (замена, перестановка, лишняя или пропущенная буква); когда слово ещё не начато (курсор сразу после пробела),
  предлагаются слова, которые могут идти за предыдущим (биграммы, посчитанные по корпусу предложений Tatoeba, лицензия
  CC BY 2.0 FR). Движок предсказаний собственный — штатный плагин hunspell Qt Virtual Keyboard намеренно остаётся
  отключённым. При этом словари hunspell, установленные в системе (`/usr/share/hunspell`, `~/.local/share/hunspell`
  и другие обычные места, например пакеты `hunspell-ru` или `hunspell-en_us`), используются, если они есть: слова
  языка, для которого нет встроенного списка, берутся из них, а исправление опечатки спрашивается у них, когда свои
  словари ничего не нашли. Библиотека libhunspell загружается во время работы, поэтому hunspell — необязательная
  зависимость: без библиотеки или без установленных словарей всё работает как прежде.
  Подсказки появляются с первой буквы (порог настраивается, 1–4), чипы делаются по ширине текста и стоят слева, а
  часть, которая допишется к слову, подчёркнута (у исправлений и предсказаний подчёркивать нечего). Нажатие на
  чип (или **A** на геймпаде) применяет его: продолжение заменяет набранное слово, исправление и предсказание вставляются
  целиком, после чего **добавляется пробел**, чтобы сразу набирать следующее. Работает касанием, мышью и геймпадом
  (D-pad по ряду, **A** — применить), настраивается и отключается на вкладке *Ввод*.
...
- **Плавающая клавиатура**: клавиша со стрелками в раскладке (рядом с выбором языка) переключает клавиатуру между
  прижатой к низу панелью и плавающей. Плавающую можно тащить за свободный фон (поля вокруг клавиш) в любое место
  экрана, её ширина задаётся в процентах от ширины экрана (20–100 %, по умолчанию 80 %) и прозрачность — в процентах
  (20–100 %, по умолчанию 100 %, то есть окно полностью непрозрачно) на вкладке *Внешний вид*,
  а позиция запоминается и восстанавливается после перезапуска. Плавающая клавиатура рисуется поверх других окон,
  не сдвигает окно под собой и не убирает панель Plasma (она не занимает низ экрана).
- **Страница настроек** (`plasma-keyboard-custom` в Параметрах системы), организованная по вкладкам — *Раскладки* (языки),
  ...
  *Ввод* (автозаглавные буквы, подсказки слов, альтернативные символы, звук, вибрация, навигация, тестовое поле):
  ...
  - ширина плавающей клавиатуры в процентах от ширины экрана (20–100 %),
  - прозрачность плавающей клавиатуры в процентах (20–100 %, 100 % — полностью непрозрачно),
  ...
  - открытие по долгому нажатию с его порогом, ряд F1–F12, шрифт клавиатуры, скрытие панели Plasma, пока
    клавиатура видна,
  - подсказки слов: включение, сколько подсказок показывать одновременно (1–5), со скольких букв они появляются (1–4),
    предсказание следующего слова и исправление опечаток,
...
Plasma Keyboard (custom) рисует клавиатуру из палитры. С ней поставляются десять тем, и вы можете добавить свои в виде JSON-файлов. Тема несёт только цвета и несколько значений геометрии/стиля, поэтому файл темы — это **данные, а не код** — импорт файла, написанного кем-то другим, не может ничего выполнить.
...
| `catppuccin-mocha` | Catppuccin Mocha | Тёмная палитра [Catppuccin Mocha](https://catppuccin.com/palette): почти чёрная панель, клавиши-буквы цвета mantle, спецклавиши surface0 и голубая клавиша действия; плоские клавиши без контура и тени, узкие зазоры между ними. |
| `google-light` | Google Light | Gboard в светлых цветах: белые клавиши на светло-серой панели, светло-серые спецклавиши и клавиша действия Google Blue; плоские круглые клавиши с узкими зазорами. |
| `google-dark` | Google Dark | Gboard в тёмных цветах: серо-синие клавиши на более тёмной панели, тёмные спецклавиши и бирюзовая клавиша действия; плоские круглые клавиши с узкими зазорами. |
...
| `base` | string | Встроенная тема, из которой берутся неуказанные значения. Необязательно, по умолчанию `system`; если задано, должно быть одним из десяти id выше. |
...
## Голосовой ввод (локальное распознавание речи)

Клавиатура умеет распознавать речь **локально**: звук никуда не отправляется, интернет не нужен. Функция выключена по умолчанию и включается на вкладке **Голосовой ввод** в настройках виртуальной клавиатуры (там же скачиваются модели).

Движки (переключаются в настройках):

| Движок | Что это | Язык |
|---|---|---|
| **Parakeet v3** (по умолчанию) | NVIDIA Parakeet TDT 0.6B v3, работает через `libparakeet` из whisper.cpp; быстро и с качеством больших моделей | 25 языков, включая русский и английский; язык определяется сам |
| **Whisper** | whisper.cpp (`libwhisper`), классическая модель | язык берётся из раскладки клавиатуры или задаётся в настройках |
| **Vosk** | `libvosk` из пакета `vosk-api`; самые маленькие модели | язык определяется моделью |

Модели скачиваются кнопкой **Скачать** на той же вкладке (и удаляются кнопкой **Удалить**) в `~/.local/share/plasma-keyboard/stt/<движок>/<модель>/`. Доступны: Parakeet v3 q8_0/q4_k, Whisper small/medium, модели Vosk для русского, английского, немецкого, французского, испанского, итальянского, украинского и португальского.

Как пользоваться: рядом с пробелом появляется **кнопка микрофона**. Она открывает голосовой режим — большая кнопка микрофона по центру; первый тап начинает запись, второй останавливает её и вставляет распознанный текст в активное поле. Режим остаётся открытым, пока вы сами не выйдете из него стрелкой в правом верхнем углу (рядом — кнопка скрытия клавиатуры). Под микрофоном оставлены нужные при диктовке клавиши: переключение раскладки, пробел (с названием языка), Enter и Backspace.

Геймпадом голосовой режим управляется так же, как обычная раскладка: **D-pad** двигает подсветку по семи элементам — большой кнопке микрофона, ряду под ней (раскладка, пробел, Enter, Backspace) и угловым кнопкам (скрыть клавиатуру и вернуться к раскладке); **A** нажимает выделенное, **B** выходит из голосового режима к обычной раскладке, **Start** скрывает клавиатуру, а **X**/**Y**/**RT**/**RB** сохраняют обычное значение (backspace, пробел, Enter, переключение раскладки). На каждой кнопке нарисован глиф той кнопки геймпада, которая её нажимает.

Примечания:

- Whisper и Parakeet собираются вместе с клавиатурой (исходники whisper.cpp, лицензия MIT) — отдельные пакеты не нужны.
- Vosk работает, только если в системе установлен `vosk-api` (`libvosk`); без него движок просто недоступен, остальные продолжают работать.
- Лицензии моделей: Parakeet v3 — CC-BY-4.0, Whisper — MIT, Vosk — Apache-2.0.
...
Иконки клавиатуры взяты из двух тем: большинство — из
[Papirus](https://github.com/PapirusDevelopmentTeam/papirus-icon-theme) (© Papirus Development
Team, GPL-3.0-only), а те, которых в Papirus нет (состояния Shift, скрытие клавиатуры и режим
выделения текста), — из Breeze (© участники KDE, LGPL-3.0-or-later).
