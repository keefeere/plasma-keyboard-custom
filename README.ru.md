...
- **Темы**: восемь встроенных тем (system, светлая/тёмная, а также iOS и Material в светлом и тёмном вариантах, а также Catppuccin Mocha) плюс
...
Plasma Keyboard (custom) рисует клавиатуру из палитры. С ней поставляются восемь тем, и вы можете добавить свои в виде JSON-файлов. Тема несёт только цвета и несколько значений геометрии/стиля, поэтому файл темы — это **данные, а не код** — импорт файла, написанного кем-то другим, не может ничего выполнить.
...
| `base` | string | Встроенная тема, из которой берутся неуказанные значения. Необязательно, по умолчанию `system`; если задано, должно быть одним из восьми id выше. |
